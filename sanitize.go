...
func HTMLAllowing(s string, args ...[]string) (string, error) {
	...
	for {
		...
		case parser.StartTagToken:
			...
			if token.Data == "a" {
				// mamy link, wiec wyciagamy href'a z niego, parsujemy link, jesli sie uda to zwracamy tylko domene i jakas koncowke (bez parametrow query)
		...
		case parser.TextToken:
			...
			if ignore == "" {
				splitted := strings.Split(token.Data, " ")
				for _, text := range splitted {
					if strings.HasPrefix(text, "http://") || strings.HasPrefix(text, "https://") {
						// prawdopodobnie mamy url, wiec probujemy parsowac, jak sie uda to wyciagamy domenke i bez kropek ja dodajemy do tresci
